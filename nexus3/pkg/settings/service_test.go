...
func getTestService() *SettingsService {
	return NewSettingsService(getTestClient())
}
...
func TestNewSettingsService(t *testing.T) {
	...
	assert.NotNil(t, s, "NewSettingsService() must not return nil")
	assert.NotNil(t, s.HTTP, "NewSettingsService() must initialize the HTTP service")
}
