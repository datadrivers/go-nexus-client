...
func TestP2ProxyRepository(t *testing.T) {
	...
	repo := getTestP2ProxyRepository("test-bower-repo-hosted")

	err := service.P2.Proxy.Create(repo)
	assert.Nil(t, err)
	generatedRepo, err := service.P2.Proxy.Get(repo.Name)
	assert.Nil(t, err)
	...
	err = service.P2.Proxy.Update(repo.Name, updatedRepo)
	assert.Nil(t, err)
	generatedRepo, err = service.P2.Proxy.Get(updatedRepo.Name)
	assert.Nil(t, err)
	assert.Equal(t, updatedRepo.Online, generatedRepo.Online)

	service.P2.Proxy.Delete(repo.Name)
	assert.Nil(t, err)
}
