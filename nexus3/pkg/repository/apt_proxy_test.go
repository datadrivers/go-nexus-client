...
func TestAptProxyRepository(t *testing.T) {
	...
	err := service.Apt.Proxy.Create(repo)
	assert.Nil(t, err)
	generatedRepo, err := service.Apt.Proxy.Get(repo.Name)
	assert.Nil(t, err)
	...
	err = service.Apt.Proxy.Update(repo.Name, updatedRepo)
	assert.Nil(t, err)
	generatedRepo, err = service.Apt.Proxy.Get(updatedRepo.Name)
	assert.Nil(t, err)
	assert.Equal(t, updatedRepo.Online, generatedRepo.Online)
	assert.Equal(t, updatedRepo.Apt, generatedRepo.Apt)

	service.Apt.Proxy.Delete(repo.Name)
	assert.Nil(t, err)
}
