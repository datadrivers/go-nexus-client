...
func getTestAlpineGroupRepository(name string) repository.AlpineGroupRepository {
	return repository.AlpineGroupRepository{
		Name:   name,
		Online: true,
...
func TestAlpineGroupRepository(t *testing.T) {
	service := getTestService()
	repo := getTestAlpineGroupRepository("test-alpine-repo-group-" + strconv.Itoa(rand.Intn(1024)))

	testProxyRepo := getTestAlpineProxyRepository("test-alpine-group-proxy-" + strconv.Itoa(rand.Intn(1024)))
