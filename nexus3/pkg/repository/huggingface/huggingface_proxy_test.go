...
func getTestHuggingfaceProxyRepository(name string) repository.HuggingfaceProxyRepository {
	return repository.HuggingfaceProxyRepository{
		Name:   name,
		Online: true,
...
func TestHuggingfaceProxyRepository(t *testing.T) {
	service := getTestService()
	repo := getTestHuggingfaceProxyRepository("test-huggingface-repo-hosted-" + strconv.Itoa(rand.Intn(1024)))
