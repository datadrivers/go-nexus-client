...
func TestConanProxyRepository(t *testing.T) {
	...
	repo := getTestConanProxyRepository("test-bower-repo-hosted")

	err := service.Conan.Proxy.Create(repo)
	assert.Nil(t, err)
	generatedRepo, err := service.Conan.Proxy.Get(repo.Name)
	assert.Nil(t, err)
	...
	err = service.Conan.Proxy.Update(repo.Name, updatedRepo)
	assert.Nil(t, err)
	generatedRepo, err = service.Conan.Proxy.Get(updatedRepo.Name)
	assert.Nil(t, err)
	assert.Equal(t, updatedRepo.Online, generatedRepo.Online)

	service.Conan.Proxy.Delete(repo.Name)
	assert.Nil(t, err)
}
