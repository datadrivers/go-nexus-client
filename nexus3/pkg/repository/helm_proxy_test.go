...
func TestHelmProxyRepository(t *testing.T) {
	...
	repo := getTestHelmProxyRepository("test-bower-repo-hosted")

	err := service.Helm.Proxy.Create(repo)
	assert.Nil(t, err)
	generatedRepo, err := service.Helm.Proxy.Get(repo.Name)
	assert.Nil(t, err)
	...
	err = service.Helm.Proxy.Update(repo.Name, updatedRepo)
	assert.Nil(t, err)
	generatedRepo, err = service.Helm.Proxy.Get(updatedRepo.Name)
	assert.Nil(t, err)
	assert.Equal(t, updatedRepo.Online, generatedRepo.Online)

	service.Helm.Proxy.Delete(repo.Name)
	assert.Nil(t, err)
}
