...
func TestMavenProxyRepository(t *testing.T) {
	...
	err := service.Maven.Proxy.Create(repo)
	assert.Nil(t, err)
	generatedRepo, err := service.Maven.Proxy.Get(repo.Name)
	assert.Nil(t, err)
	...
	updatedRepo.Maven.LayoutPolicy = &newLayoutPolicy

	err = service.Maven.Proxy.Update(repo.Name, updatedRepo)
	assert.Nil(t, err)
	generatedRepo, err = service.Maven.Proxy.Get(updatedRepo.Name)
	assert.Nil(t, err)
	...
	assert.Equal(t, updatedRepo.Maven.LayoutPolicy, generatedRepo.Maven.LayoutPolicy)

	service.Maven.Proxy.Delete(repo.Name)
	assert.Nil(t, err)
}
