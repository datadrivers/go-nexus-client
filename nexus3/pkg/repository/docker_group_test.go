...
func TestDockerGroupRepository(t *testing.T) {
	...
	defer service.Docker.Proxy.Delete(testProxyRepo.Name)
	err := service.Docker.Proxy.Create(testProxyRepo)
	assert.Nil(t, err)
	repo.Group.MemberNames = append(repo.Group.MemberNames, testProxyRepo.Name)

	err = service.Docker.Group.Create(repo)
	assert.Nil(t, err)
	generatedRepo, err := service.Docker.Group.Get(repo.Name)
	assert.Nil(t, err)
	...
	updatedRepo.Docker.ForceBasicAuth = false

	err = service.Docker.Group.Update(repo.Name, updatedRepo)
	assert.Nil(t, err)
	generatedRepo, err = service.Docker.Group.Get(updatedRepo.Name)
	assert.Nil(t, err)
	assert.Equal(t, updatedRepo.Online, generatedRepo.Online)
	assert.Equal(t, updatedRepo.Docker.ForceBasicAuth, generatedRepo.Docker.ForceBasicAuth)

	service.Docker.Group.Delete(repo.Name)
	assert.Nil(t, err)
}
