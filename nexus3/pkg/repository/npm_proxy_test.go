...
func TestNpmProxyRepository(t *testing.T) {
	...
	err := service.Npm.Proxy.Create(repo)
	assert.Nil(t, err)
	generatedRepo, err := service.Npm.Proxy.Get(repo.Name)
	assert.Nil(t, err)
	...
	updatedRepo.Npm.RemoveQuarantined = true

	err = service.Npm.Proxy.Update(repo.Name, updatedRepo)
	assert.Nil(t, err)
	generatedRepo, err = service.Npm.Proxy.Get(updatedRepo.Name)
	assert.Nil(t, err)
	assert.Equal(t, updatedRepo.Online, generatedRepo.Online)
	assert.Equal(t, updatedRepo.Npm, generatedRepo.Npm)

	service.Npm.Proxy.Delete(repo.Name)
	assert.Nil(t, err)
}
