...
func TestCondaProxyRepository(t *testing.T) {
	...
	repo := getTestCondaProxyRepository("test-bower-repo-hosted")

	err := service.Conda.Proxy.Create(repo)
	assert.Nil(t, err)
	generatedRepo, err := service.Conda.Proxy.Get(repo.Name)
	assert.Nil(t, err)
	...
	err = service.Conda.Proxy.Update(repo.Name, updatedRepo)
	assert.Nil(t, err)
	generatedRepo, err = service.Conda.Proxy.Get(updatedRepo.Name)
	assert.Nil(t, err)
	assert.Equal(t, updatedRepo.Online, generatedRepo.Online)

	service.Conda.Proxy.Delete(repo.Name)
	assert.Nil(t, err)
}
