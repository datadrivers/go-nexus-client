...
func TestDockerProxyRepository(t *testing.T) {
	...
	repo := getTestDockerProxyRepository("test-docker-repo-hosted")

	err := service.Docker.Proxy.Create(repo)
	assert.Nil(t, err)
	generatedRepo, err := service.Docker.Proxy.Get(repo.Name)
	assert.Nil(t, err)
	...
	err = service.Docker.Proxy.Update(repo.Name, updatedRepo)
	assert.Nil(t, err)
	generatedRepo, err = service.Docker.Proxy.Get(updatedRepo.Name)
	assert.Nil(t, err)
	assert.Equal(t, updatedRepo.Online, generatedRepo.Online)
	assert.Equal(t, updatedRepo.Docker, generatedRepo.Docker)

	service.Docker.Proxy.Delete(repo.Name)
	assert.Nil(t, err)
}
